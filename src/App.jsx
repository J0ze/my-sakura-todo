...
import { Trash2, Check, Plus, Flower, Sparkles, Cloud, AlertCircle } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

// ---------------------------------------------------------
// 1. 你的 Firebase 配置
// 警告：如果你还在用之前的项目ID (sakura-todo-de8a7)，
// 它可能因为选错地区而一直报错 "Requires billing"。
// 强烈建议新建一个项目 (选 us-central1) 并替换这里的配置。
// ---------------------------------------------------------
    ...
    <div className="p-4 bg-white/50 border-b border-gray-100">
      <div className="relative flex items-center">
        <input
          ...
          placeholder={isReady ? "添加新的待办事项..." : "正在连接云端..."}
          disabled={!isReady}
          className={`w-full pl-4 pr-12 py-3 rounded-full bg-white border focus:outline-none focus:ring-2 transition-all shadow-sm text-gray-600 placeholder-gray-400 disabled:bg-gray-100 disabled:cursor-not-allowed
            ${side === 'left' 
        ...
        <button
          ...
          className={`absolute right-2 p-2 rounded-full text-white transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:hover:scale-100 disabled:cursor-not-allowed
...
export default function App() {
  ...
  const [errorMsg, setErrorMsg] = useState(null); // 新增：错误信息状态
  ...
  useEffect(() => {
    ...
    const unsubscribeSnapshot = onSnapshot(q, (snapshot) => {
      ...
      if (error.message.includes("billing")) {
          setErrorMsg("错误：该 Firebase 项目需要付费才能使用。请新建一个项目并在 Location 处选择 us-central1。");
      } else if (error.message.includes("permission")) {
          setErrorMsg("错误：没有权限。请检查 Firestore Rules 是否已设置为 Test Mode。");
      } else {
          setErrorMsg("数据连接错误: " + error.message);
      }
      setLoading(false);
    });
  ...
  const handleAddTodo = async (side, text, setText) => {
    ...
    try {
      ...
      alert("添加失败！详细错误请按 F12 查看 Console。\n\n可能原因：" + error.message);
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-pink-50 flex-col gap-4">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-pink-400"></div>
        <p className="text-gray-400 text-sm">正在连接云端数据库...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#fdf2f8] selection:bg-pink-100 relative font-sans overflow-hidden">
      {/* Error Banner */}
      {errorMsg && (
        <div className="absolute top-0 left-0 w-full bg-red-500 text-white p-3 text-center z-50 shadow-md flex items-center justify-center gap-2">
            <AlertCircle size={20} />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-8 h-screen flex flex-col">
        {/* Main Title */}
        <header className="text-center mb-8 flex-shrink-0 mt-4">
